...
- **$ \mathbf{P_C} $:** Price of the asset on the CEX
- **$ \mathbf{P_D} $:** Price of the asset on the DEX
- **$ \mathbf{f_C} $:** Trading fee rate on the CEX
- **$ \mathbf{f_D} $:** Trading fee rate on the DEX
- **$ \mathbf{G} $:** Gas cost for DEX transactions
- **$ \mathbf{Q} $:** Trade quantity
- **$ \mathbf{Q_{C,max}} $:** Maximum trade size on CEX
- **$ \mathbf{Q_{D,max}} $:** Maximum trade size on DEX (liquidity-constrained)
- **$ \mathbf{\Delta P} $:** Price difference $ \Delta P = |P_C - P_D| $
- **$ \mathbf{\sigma_C} $:** Price volatility on CEX
- **$ \mathbf{\sigma_D} $:** Price volatility on DEX
- **$ \mathbf{\rho} $:** Correlation coefficient between CEX and DEX prices
- **$ \mathbf{E_C} $:** Position exposure on CEX
- **$ \mathbf{E_D} $:** Position exposure on DEX
- **$ \mathbf{E_{net}} $:** Net exposure across platforms $ E_{net} = E_C + E_D $
- **$ \mathbf{\tau_C} $:** Execution delay on CEX
- **$ \mathbf{\tau_D} $:** Execution delay on DEX (block confirmation time)
- **$ \mathbf{C_{total}} $:** Total capital available for the strategy
