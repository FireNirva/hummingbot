...
- **$ \mathbf{P_i} $:** DEX $i$ 上的资产价格
- **$ \mathbf{f_i} $:** DEX $i$ 上的交易费率
- **$ \mathbf{G} $:** gas 成本（以 gas 单位计）
- **$ \mathbf{G_p} $:** gas 价格（以 ETH/gas 单位计）
- **$ \mathbf{S_i} $:** DEX $i$ 上的滑点函数
- **$ \mathbf{R^x_i}, \mathbf{R^y_i} $:** DEX $i$ 上的资产储备
- **$ \mathbf{k_i} $:** DEX $i$ 的常数乘积（对于 $x \cdot y = k$ 类型的 AMM）
- **$ \mathbf{Q} $:** 交易量
- **$ \mathbf{\tau} $:** 执行延迟（以区块计）
- **$ \mathbf{\sigma} $:** 价格波动率
- **$ \mathbf{\lambda} $:** 风险回避系数
- **$ \mathbf{MEV_{cost}} $:** 预执行交易风险的估计成本
