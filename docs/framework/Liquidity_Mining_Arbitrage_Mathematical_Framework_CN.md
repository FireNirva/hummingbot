...
- **$ \mathbf{P_B} $:** 资产 B 的价格
- **$ \mathbf{P_R} $:** 奖励代币的价格
- **$ \mathbf{Q_A} $:** 流动性池中资产 A 的数量
- **$ \mathbf{Q_B} $:** 流动性池中资产 B 的数量
- **$ \mathbf{Q_R} $:** 奖励代币的数量
- **$ \mathbf{L} $:** 提供的流动性（以美元计）
- **$ \mathbf{L_{total}} $:** 流动性池中的总流动性
- **$ \mathbf{s} $:** 提供者在池中的份额 ($ s = \frac{L}{L_{total}} $)
- **$ \mathbf{f_{pool}} $:** 流动性池收取的交易费率
- **$ \mathbf{f_{ext}} $:** 提供流动性或提款的外部费用
- **$ \mathbf{APY_{pool}} $:** 来自交易费用的年化收益率
- **$ \mathbf{APY_{rewards}} $:** 来自代币奖励的年化收益率
- **$ \mathbf{APY_{total}} $:** 总年化收益率
- **$ \mathbf{r} $:** 发放的奖励率（代币/美元/时间单位）
- **$ \mathbf{T} $:** 投资时间范围（天）
- **$ \mathbf{\sigma_A} $:** 资产 A 的价格波动率
- **$ \mathbf{\sigma_B} $:** 资产 B 的价格波动率
- **$ \mathbf{\sigma_R} $:** 奖励代币的价格波动率
- **$ \mathbf{\rho_{AB}} $:** 资产 A 和 B 之间的价格相关性
- **$ \mathbf{IL} $:** 无常损失（以百分比计）
