...
- **$ \mathbf{r_{LP}} $:** Base yield rate from liquidity provision (from trading fees)
- **$ \mathbf{r_{RM}} $:** Reward token mining yield rate
- **$ \mathbf{IL} $:** Impermanent loss
- **$ \mathbf{r_{ARB}} $:** Yield rate from arbitrage strategy
- **$ \mathbf{x}, \mathbf{y} $:** Token reserves in the liquidity pool
- **$ \mathbf{k} $:** Constant product $ k = x \cdot y $
- **$ \mathbf{p} $:** Price ratio $ p = \frac{y}{x} $
- **$ \mathbf{p_0} $:** Initial price ratio
- **$ \mathbf{p_M} $:** External market price
- **$ \mathbf{L} $:** Liquidity depth $ L = \sqrt{k} $
- **$ \mathbf{f} $:** Liquidity pool fee rate
- **$ \mathbf{C_{LP}} $:** Capital allocated to liquidity provision
- **$ \mathbf{C_{ARB}} $:** Capital allocated to arbitrage
- **$ \mathbf{C_{total}} $:** Total available capital
- **$ \mathbf{G} $:** Gas cost of transactions
- **$ \mathbf{\alpha} $:** Capital allocation ratio $ \alpha = \frac{C_{LP}}{C_{total}} $
- **$ \mathbf{\sigma} $:** Price volatility
- **$ \mathbf{T} $:** Investment time horizon
