...
2. 创建配置：
   ...
   market_2: ETH-USDT
   wallet_address: "0xYourBaseWalletForThisBot"
   pool_id: [如有需要填写特定池ID]
   order_amount: 0.05
...
3. 多 Bot 风险控制：
   - 如果多个 Bot 都在同一条 EVM 链上通过 Gateway 发单，不要共用同一个钱包
   - 推荐结构是 `多个钱包 + 一个 Gateway`
   - 每个活跃 Bot 单独配置自己的 `wallet_address`
...
2. 钱包分配原则：
   - 一个 Base 钱包对应一个活跃执行 Bot
   - 多个 Bot 可以共享同一个 Gateway 服务
   - 不要让多个活跃 Bot 依赖同一个 Gateway 默认钱包，否则容易出现 nonce 冲突
